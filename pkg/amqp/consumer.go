...
type ConsumeHandler func(string, <-chan mq.Delivery, chan error)

func (p *Consumer) Consume(f ConsumeHandler) (err error) {
	ch := p.ch
	...
	go f(p.options.RoutingKey, deliveries, p.done)
	return nil
}
